            ...
            <form id="add_form">
                ...
                <div class="box box-success">
                    <div class="box-body">
                        ...
                        <button data-target="#pc_items_table" uri="{{route('dashboard.ajax.get','add_row')}}?view=pc_items" style="margin-bottom: 5px; margin-top: 5px; margin-right: 5px" type="button" id="add_button" class="btn btn-xs btn-success pull-right add_button"><i class="fa fa-plus"></i> Add item</button>
                    ...
                    <div class="row">
                        <div class="col-md-12" style="min-height: 200px">
                            <table id="pc_items_table" class="table-bordered table table-condensed table-striped">
                                ...
                                <tbody>
                                @foreach($propCardDetails as $item)
                                    <tr id="{{ $item->transaction_slug }}">
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][date]" name="items['{{ $item->transaction_slug }}'][date]" type="text" value="{{ $item->date }}"></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][ref_no]" name="items['{{ $item->transaction_slug }}'][ref_no]" type="text" value="{{ $item->ref_no }}"></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][receipt_qty]" name="items['{{ $item->transaction_slug }}'][receipt_qty]" type="text" value="{{ $item->receipt_qty }}"></td>
                                        <td><textarea class="input-sm" id="items['{{ $item->transaction_slug }}'][qty]" name="items['{{ $item->transaction_slug }}'][qty]" type="text">{{ $item->qty }}</textarea></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][purpose]" name="items['{{ $item->transaction_slug }}'][purpose]" type="text" value="{{ $item->purpose }}"></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][bal_qty]" name="items['{{ $item->transaction_slug }}'][bal_qty]" type="text" value="{{ $item->bal_qty }}"></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][amount]" name="items['{{ $item->transaction_slug }}'][amount]" type="text" value="{{ $item->amount }}"></td>
                                        <td><input class="form-control" id="items['{{ $item->transaction_slug }}'][remarks]" name="items['{{ $item->transaction_slug }}'][remarks]" type="text" value="{{ $item->remarks }}"></td>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#add_button').click(function () {
                ...
                let bal_qty = fieldset.find('input[name="receipt_qty"]').val();
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        Swal.fire({
                            ...
                            icon: 'success',
                            html: 'Click the print button below to print.',
